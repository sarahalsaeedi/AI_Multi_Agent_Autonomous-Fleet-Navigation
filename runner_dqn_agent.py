...
from learning.DQNA import DQNAgent
    ...
    def restart(self, args):
        ...
        while self.player is None:
            if not self.map.get_spawn_points():
                ...
                sys.exit(1)
            spawn_points = self.map.get_spawn_points()[30]
            spawn_points2 = self.map.get_spawn_points()
            spawn_point = random.choice(spawn_points2) if spawn_points else carla.Transform()
            # print("Spawning the player", spawn_point)
            self.player = self.world.try_spawn_actor(blueprint, spawn_points)
        # Set up the sensors.
        self.collision_sensor = CollisionSensor(self.player, self.hud)
        self.lane_invasion_sensor = LaneInvasionSensor(self.player, self.hud, self.control_option)
        self.gnss_sensor = GnssSensor(self.player, self.hud, self.control_option)
        self.obstacle_sensor = ObstaclesSensor(self.player, self.hud, self.control_option)
...
class ControlOptions:
    ...
    def get_control_option(self, control):
        if control == "obs":
            ...
            return self.lane_line, self.obs_time
        elif control == "Red":
...
class ObstaclesSensor(object):
    ...
    def __init__(self, parent_actor, hud, control_option):
        ...
        # reference.
        weak_self = weakref.ref(self)
        self.sensor.listen(lambda event: ObstaclesSensor._on_detect(weak_self, event, control_option))
    ...
    def measure_obs_distance(self, event, control_option):
        ...
        if not "roadline" or "road" or "ride" in event.other_actor.type_id:
            ...
            if event.distance < 13:
                # send the agent flag to stop driving
                # print("force stop")
                self.hud.notification('obstacle: %r is near -> Emergency Stop' % actor_name)
                ...
                control_option.set_control_option("obs", False, time.time())
                self.hud.notification('obstacle: %r moved -> Save to go' % actor_name)
...
class LaneInvasionSensor(object):
    ...
    def __init__(self, parent_actor, hud, control_option):
        ...
        weak_self = weakref.ref(self)
        self.sensor.listen(
            lambda event: LaneInvasionSensor._on_invasion(weak_self, event, parent_actor, control_option))
    ...
    @staticmethod
    def _on_invasion(weak_self, event, parent_actor, control_option):
        """On invasion method"""
        self = weak_self()
        if not self:
            return
...
class GnssSensor(object):
    ...
    def __init__(self, parent_actor, hud, control_option):
        ...
        weak_self = weakref.ref(self)
        self.sensor.listen(
            lambda event: GnssSensor._on_gnss_event(weak_self, hud, event, self.world, control_option, parent_actor))
        ...
        if parent_actor.is_alive:
            ...
            if parent_actor.is_at_traffic_light():
                ...
                if str(traffic_light.get_state()) == "Red":
                    ...
                    control_option.set_control_option("Red", True, time.time())
                else:
            ...
            if str(lane_type) == "Sidewalk":
                ...
                control_option.set_control_option("sidewalk", True, time.time())
            else:
                control_option.set_control_option("sidewalk", False, time.time())
        else:
...
def run_agent(args, reward, learner, load_path, training_path, episode):
    """ Main Agent Runner"""
    ...
    learner.get_qs(np.ones((IM_HEIGHT, IM_WIDTH, 3)))

    for running_episode in range(1000):
        pygame.init()
        pygame.font.init()
        world = None
        ...
        try:
            ...
            final_spawn_point = world.map.get_spawn_points()[5]
            # agent.set_destination(
            #      (final_spawn_point.location.x, final_spawn_point.location.y, final_spawn_point.location.z)
            #     )
            ...
            # tf.compat.v1.esable_eager_execution()
            # writer = tf.compat.v1.summary.FileWriter(training_path, tf.compat.v1.get_default_graph())
            ...
            while not done:
                ...
                pygame.display.flip()
                current_state = world.camera_manager.front_camera
                world.player.apply_control(carla.VehicleControl(throttle=0.4, brake=0.0))
                if np.random.random() > epsilon:
                # print('Action from Qtable')
                # Get action from Q table
                    action = np.argmax(learner.get_qs(current_state))
                else:

                    action = np.random.randint(0, 4)
                option = check_conditions(world)

                agent_control, new_state, new_reward, done1, apply, _ = run_step(hud, clock_run, world, action, option,
                                                                                 clock)
                learner.update_replay_memory(
            ...
            running_episode += 1
            print('epsilon ', epsilon, 'EP score ', score, 'EPISODE', running_episode)
            if  running_episode % 10 == 0:
                print("running_episode", running_episode)

                learner.train_in_loop()
                learner.model.save('models/DQN_%s_july.model' % score)
                try:
...
def run_step(hud, clock_run, world, action, option, clock):
    ...
    lane_line, obs_time2 = world.control_option.get_control_option("lane")
    is_stop, obs_time3 = world.control_option.get_control_option("stop")
    # hud.tick(world, clock, 'sarah', 'name')

    if len(colhist) != 0:
        # hud.tick(world, clock, 0, 1, 0)
        done = True
        reward = -200
    elif is_sidewalk:
        # hud.tick(world, clock, 0, 1, 0)
    ...
    return control, world.camera_manager.front_camera, reward, done, apply, None
...
def set_action(world, action, option):
    ...
    if option == "Stop":
        if action == 0:
            ...
            # print("sorry I have to stop you")
            control = carla.VehicleControl()
            control.steer = 0.0
            control.throttle = 0.0
            control.brake = 1.0
...
def main():
    ...
    argparser = argparse.ArgumentParser(
        description=' CAU DQN Learning Agent ')
    ...
    reward = 1
    try:
        run_agent(args, reward, learner, '', '', episode=1000)
